...
interface Store { gameId: string | null; state: GameState; storage: KeyValueStorage; dispatch: (intent: Intent) => { ok: true } | { ok: false; error: string }; createGame: (config: GameConfig, accounts: Account[]) => string; loadGame: (id: string) => boolean; listGames: () => ArchiveEntry[]; saveDraft: (value: unknown) => void; loadDraft: <T>() => T | null; }
