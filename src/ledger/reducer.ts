...
export const initialState = (): GameState => ({ started: false, gameStarted: false, ended: false, accounts: {}, balances: {}, eliminated: new Set(), reversed: new Set(), events: [], settlements: {}, settlementStatus: {}, settlementStarted: false, jailed: new Set(), lostInCirculation: 0, requests: {}, invalid: false });
...
// Banker authority is stricter than ordinary player authority: only the Host's
...
export function applyEvent(previous: GameState, input: GameEvent): GameState { const parsed = parseEvent(input); if (!parsed) return { ...previous, invalid: true }; if (previous.events.some(e => e.intentId === parsed.intentId)) return previous; if (parsed.seq !== previous.events.length) return { ...previous, invalid: true }; const s = copy(previous); const fail = () => ({ ...previous, invalid: true });
  switch (parsed.type) {
    ...
    case 'players.reordered': { const { playerIds } = parsed.payload; const active = activePlayers(s); const currentIds = active.map(player => player.id); const nextIds = new Set(playerIds); if (!s.started || s.ended || s.gameStarted || !isHost(s, parsed.actorId) || playerIds.length !== currentIds.length || nextIds.size !== currentIds.length || currentIds.some(id => !nextIds.has(id))) return fail(); const ordered: Record<string, Account> = {}; Object.values(s.accounts).filter(account => account.kind !== 'player').forEach(account => { ordered[account.id] = account; }); playerIds.forEach(id => { ordered[id] = s.accounts[id]; }); s.accounts = ordered; s.currentTurnAccountId = playerIds[0]; break; }
    case 'game.begun': { if (!s.started || s.ended || s.gameStarted || !isHost(s, parsed.actorId) || activePlayers(s).length < 2) return fail(); s.gameStarted = true; break; }
    ...
    // Any player may jail/release themselves; only the Banker may jail/release
    // another player.
    case 'player.jailed': { const id = parsed.payload.accountId; const a = s.accounts[id]; if (!s.started || s.ended || !a || a.kind !== 'player' || s.eliminated.has(id) || (parsed.actorId !== id && !isBanker(s, parsed.actorId))) return fail(); s.jailed.add(id); break; }
    case 'player.released': { const id = parsed.payload.accountId; const a = s.accounts[id]; if (!s.started || s.ended || !a || a.kind !== 'player' || s.eliminated.has(id) || (parsed.actorId !== id && !isBanker(s, parsed.actorId))) return fail(); s.jailed.delete(id); break; }
