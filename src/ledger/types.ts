...
export type EventType = 'game.started' | 'player.joined' | 'player.renamed' | 'transfer' | 'transfer.reversed' | 'player.eliminated' | 'settlement.started' | 'settlement.submitted' | 'settlement.dismissed' | 'game.ended' | 'turn.advanced' | 'player.jailed' | 'player.released';
...
export interface GameState { started: boolean; ended: boolean; config?: GameConfig; accounts: Record<string, Account>; balances: Record<string, number>; eliminated: Set<string>; reversed: Set<number>; events: GameEvent[]; tally?: Settlement[]; settlements: Record<string, Settlement>; settlementStatus: Record<string, SettlementStatus>; settlementStarted: boolean; hostAccountId?: string; lastPayer?: string; currentTurnAccountId?: string; jailed: Set<string>; lostInCirculation: number; invalid: boolean; }
export const DEFAULT_CONFIG: GameConfig = { startingCash: 1500, goSalary: 200, doubleOnExactGo: false, freeParkingPot: false, bankerMode: true, currencySymbol: '$', quickAmounts: [1, 10, 20, 50, 100, 200, 500] };
